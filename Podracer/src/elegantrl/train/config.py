...
class Arguments:
    def __init__(
        self, agent_class=None, env=None, env_func=None, env_args: dict = None
    ):
    ...
    def init_before_training(self):
        ...
        if self.cwd is None:
            self.cwd = (
                f"./{self.env_name}_{self.agent_class.__name__[5:]}_{self.learner_gpus}"
            )
...
def get_gym_env_args(env, if_print=True) -> dict:  # [ElegantRL.2021.12.12]
    ...
    target_return = (
        getattr(env, "target_return") if hasattr(env, "target_return") else +np.inf
    )
