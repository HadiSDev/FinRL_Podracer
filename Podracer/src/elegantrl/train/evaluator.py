...
class Evaluator:
    ...
    def evaluate_save_and_plot(
        self, act, steps: int, r_exp: float, log_tuple: tuple
    ) -> (bool, bool):
        ...
        if time.time() - self.eval_time < self.eval_gap:
            ...
            """evaluate first time"""
            rewards_steps_list = [
                get_cumulative_returns_and_step(self.eval_env, act)
            ...
            if if_save:  # save checkpoint with highest episode return
                ...
                act_path = (
                    f"{self.cwd}/actor__{self.total_step:012}_{self.r_max:09.3f}.pth"
                )
...
def get_cumulative_returns_and_step(env, act) -> (float, int):  # [ElegantRL.2022.03.03]
    ...
    for steps in range(max_step):
        ...
        tensor_action = (
            act(tensor_state).argmax(dim=1) if if_discrete else act(tensor_state)
        )
        action = (
...
def save_learning_curve(
    recorder: list = None,
    cwd: str = ".",
    save_title: str = "learning curve",
    fig_name: str = "plot_learning_curve.jpg",
):
    if recorder is None:
