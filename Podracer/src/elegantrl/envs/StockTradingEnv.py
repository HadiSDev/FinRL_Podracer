...
class StockTradingEnv:
    ...
    def reset(self):
        ...
        if self.if_random_reset:
            ...
            self.shares = (
                np.abs(rd.randn(self.shares_num).clip(-2, +2)) * 2**6
            ).astype(int)
        else:
    ...
    def step(self, action):
        ...
        for index in range(self.action_dim):
            ...
            adj_close_price = self.close_ary[
                self.day, index
            ]  # `adjcp` denotes adjusted close price
...
class ActorPPO(nn.Module):
    def __init__(self, mid_dim, mid_layer_num, state_dim, action_dim):
        super().__init__()
        self.net = build_fcn(
            mid_dim, mid_layer_num, inp_dim=state_dim, out_dim=action_dim
        )
...
def build_fcn(
    mid_dim, mid_layer_num, inp_dim, out_dim
...
class AgentPPO:
    def __init__(self, net_dim, state_dim, action_dim, gpu_id=0, args=None):
        ...
        self.act = act_class(net_dim, self.mid_layer_num, state_dim, action_dim).to(
            self.device
        )
        self.cri = (
            cri_class(net_dim, self.mid_layer_num, state_dim, action_dim).to(
                self.device
            )
            if cri_class
            else self.act
        ...
        self.cri_optimizer = (
            torch.optim.Adam(self.cri.parameters(), self.learning_rate)
            if cri_class
    ...
    def convert_trajectory(self, traj_list, _last_done):  # [ElegantRL.2022.01.01]
        ...
        traj_list[1] = (
            torch.tensor(traj_list[1], dtype=torch.float32) * self.reward_scale
        ).unsqueeze(1)
        traj_list[2] = (
            (1 - torch.tensor(traj_list[2], dtype=torch.float32)) * self.gamma
        ).unsqueeze(1)
...
class Arguments:
    def __init__(self, agent, env_func=None, env_args=None):
        ...
        self.state_dim = self.env_args[
            "state_dim"
        ]  # vector dimension (feature number) of state
        self.action_dim = self.env_args[
            "action_dim"
        ]  # vector dimension (feature number) of action
        self.if_discrete = self.env_args[
            "if_discrete"
...
def evaluate_models_in_directory(dir_path=None):
    ...
    for model_name in model_names:
        ...
        cumulative_returns_list = [
            get_episode_return_and_step(env, actor)[0] for _ in range(4)
        ]
